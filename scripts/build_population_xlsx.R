...
#   row = floor((84 - lat) / px_res_lat) + 1    [1 = 84N edge, row increases south]
# Confirmed correct by diagnostic: Mexico City and major cities land on high values.
# ---------------------------------------------------------------------------
PX_RES_LON <- 360 / 43200   # 0.008333 deg/pixel
PX_RES_LAT <- 360 / 43200   # 0.008333 deg/pixel
LAT_TOP    <- 84
NCOLS_R    <- 43200L
NROWS_R    <- 18720L
...
extract_hex_sums_block <- function(tif_path, hex_sf) {
  ...
  for (i in seq_len(n)) {
    ...
    if ("tipo" %in% names(hex_sf) && hex_sf$tipo[i] == "Oceano") next
