...
r_fix <- raster::setExtent(r, raster::extent(-180, 180, -90, 90), keepres = TRUE)
crs(r_fix) <- CRS("+proj=longlat +datum=WGS84 +no_defs")
...
sum_hex_mask <- function(h) {
  bb <- st_bbox(h)
  h_sp <- as(h, "Spatial")
  rr <- crop(r_fix, extent(bb[["xmin"]], bb[["xmax"]], bb[["ymin"]], bb[["ymax"]]))
  mm <- mask(rr, h_sp)
  as.numeric(cellStats(mm, sum, na.rm = TRUE))
}
...
refs <- rbind(mc_hex, ocean_hex, hex[hex$hex_id == "HEX0038423", ])

for (i in seq_len(nrow(refs))) {
  h <- refs[i, ]
  v_block <- sum_hex_block(h)
  v_mask <- sum_hex_mask(h)
  cat(sprintf("%s | tipo=%s | block=%0.2f | mask_sum=%0.2f | diff=%0.2f\n",
              as.character(h$hex_id), as.character(h$tipo), v_block, v_mask, v_block - v_mask))
}
